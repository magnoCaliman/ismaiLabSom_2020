...
- Descrição
  - Entrega dos exercícios temáticos propostos ao longo do semestre, nas aulas:
    ...
    - Aula 11.1 - 2 exercícios
      - *TOTAL*: --> 8 exercícios
- Formato de entrega:
  - Arquivo zip contendo todos os arquivos de cada exercícicos, *organizados em ficheiros de acordo com o números das aulas*
  - Para os exercícios que envolvem utilização do SuperCollider, deve ser enviado o arquivo com código (.scd) juntamente do arquivo de áudio.
  - Para os exercícios que envolvem criação de sessões no Reaper, deve ser enviada pasta contendo o arquivo da sessão (.rpp) *e todos os arquivos de mídia para que a sessão seja ouvida!*.
...
- Descrição:
  ...
  - Para projetos onde não seja possível a realização física, é permitida a entrega de protótipo junto de descrição em PDF
